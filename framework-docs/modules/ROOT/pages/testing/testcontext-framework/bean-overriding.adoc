...
Typically, the bean is selected by type by default or the user directly provides the bean
name in the custom annotation. Some `BeanOverrideProcessor`s could also compute a bean
name based on a convention or another advanced method.
====
